"""Scribe rendering engine — pure functions, no I/O."""

from __future__ import annotations

import html
import re
from datetime import UTC, datetime

from .labels import Labels

# ── Proofread / recheck ──────────────────────────────────────────────────────

_PROOFREAD_SYSTEM = (
    "You are a meticulous DFIR report editor. You output ONLY the corrected text, "
    "nothing else — no preamble, no commentary."
)


def proofread(text: str, llm_call) -> str:
    """Recheck a report section to remove errors that slipped through.

    `llm_call(system, user) -> str` is injected by the caller (Scribe stays
    IO-free). The editor only CLEANS — it must not invent facts/IOCs/verdicts —
    so a failed/empty call falls back to the original text unchanged.
    """
    if not text or not text.strip() or llm_call is None:
        return text
    try:
        prompt = (
            "Proofread this investigation-report section. FIX ONLY: broken or "
            "half-rendered markdown/tables, duplicated or contradictory lines, "
            "dangling/empty references, malformed timestamps, and claims that "
            "contradict each other within the text. DO NOT add new facts, IPs, "
            "hostnames, CVEs or conclusions, and DO NOT remove real findings, "
            "fo_ids or IOCs. Preserve the structure and headings. If it's already "
            "clean, return it unchanged. Return ONLY the corrected text.\n\n"
            "----- BEGIN -----\n" + text + "\n----- END -----"
        )
        out = llm_call(_PROOFREAD_SYSTEM, prompt)
        out = (out or "").strip()
        # Guard against the model echoing the delimiters or refusing.
        for marker in ("----- BEGIN -----", "----- END -----"):
            out = out.replace(marker, "").strip()
        return out if out else text
    except Exception:
        return text

# ── Template ─────────────────────────────────────────────────────────────────

TEMPLATE_DEFAULTS = {
    "title_prefix": "Investigation report",
    "header_md": "",
    "footer_md": "_Generated by Citadel_",
    "max_flagged": 50,
    "sections": {
        "manifest": True,   # "built from" inventory header
        "exec_summary": True,
        "overview": True,   # graphical aggregates
        "modules": True,    # analysis-module run results (hayabusa/yara/…)
        "saved_searches": True,  # bookmarked queries re-run live
        "killchains": True,      # correlated reverse kill chains
        "threat_intel": True,  # enriched CTI matches table
        "ai_report": True,
        "pinned": True,
        "flagged": True,
        "mitre": True,
        "detections": False,  # noisy — the AI report surfaces the meaningful ones
        "watchlist": True,
        "notes": True,
    },
}


def merge_template(stored: dict | None) -> dict:
    """Stored template merged over defaults — missing keys fall back."""
    tpl = dict(TEMPLATE_DEFAULTS)
    tpl["sections"] = dict(TEMPLATE_DEFAULTS["sections"])
    if not isinstance(stored, dict):
        return tpl
    for k in ("title_prefix", "header_md", "footer_md"):
        if isinstance(stored.get(k), str):
            tpl[k] = stored[k]
    if isinstance(stored.get("max_flagged"), int) and stored["max_flagged"] > 0:
        tpl["max_flagged"] = min(stored["max_flagged"], 500)
    for k, v in (stored.get("sections") or {}).items():
        if k in tpl["sections"]:
            tpl["sections"][k] = bool(v)
    return tpl


# ── Shared helpers ───────────────────────────────────────────────────────────


def _ts(s: str) -> str:
    if not s:
        return ""
    try:
        return datetime.fromisoformat(str(s).replace("Z", "+00:00")).strftime(
            "%Y-%m-%d %H:%M:%S UTC"
        )
    except Exception:
        return str(s)


def _now() -> str:
    return _ts(datetime.now(UTC).isoformat())


def _source_basename(src_file) -> str:
    """`source_file` is `cases/<case>/<job>/<orig/path>`; show the original
    file's basename ("sso.log") so each event carries its provenance."""
    if not src_file or not isinstance(src_file, str):
        return ""
    s = src_file.split("?", 1)[0].split("#", 1)[0]
    m = re.match(r"^cases/[^/]+/[^/]+/(.+)$", s)
    if m:
        s = m.group(1)
    parts = [p for p in s.split("/") if p]
    return parts[-1] if parts else s


def _ev_fields(ev: dict) -> dict:
    return {
        "ts": _ts(ev.get("timestamp", "")),
        "type": ev.get("artifact_type", "?"),
        "host": (ev.get("host") or {}).get("hostname", "") or "",
        "user": (ev.get("user") or {}).get("name", "") or "",
        "msg": (ev.get("message") or "").strip().splitlines()[0][:300] if ev.get("message") else "",
        "note": ev.get("pin_note", "") or "",
        "src": _source_basename(ev.get("source_file")),
    }


# ════════════════════════════ MARKDOWN ════════════════════════════════════════


def _event_line(ev: dict) -> str:
    f = _ev_fields(ev)
    bits = [f"`{f['ts']}`" if f["ts"] else "", f"**{f['type']}**"]
    if f["host"]:
        bits.append(f"@ `{f['host']}`")
    if f["user"]:
        bits.append(f"user `{f['user']}`")
    if f["msg"]:
        bits.append(f"— {f['msg']}")
    if f["src"]:
        bits.append(f"_(from `{f['src']}`)_")
    return " ".join(b for b in bits if b)


def _agg_rows_md(title: str, items: list[dict], unit: str = "events") -> list[str]:
    if not items:
        return []
    out = [f"### {title}", ""]
    for it in items[:15]:
        out.append(f"- `{it.get('value')}` — {int(it.get('count', 0)):,} {unit}")
    out.append("")
    return out


def render_markdown(data: dict, tpl: dict | None = None, language: str | None = None) -> str:
    tpl = tpl or dict(TEMPLATE_DEFAULTS)
    L = Labels(language)
    sections = tpl.get("sections") or {}
    case = data.get("case") or {}
    pinned = data.get("pinned") or []
    flagged = data.get("flagged") or []
    mitre = data.get("mitre") or {}
    wl = data.get("watchlist") or {}
    det = data.get("detections") or {}
    notes = data.get("notes") or ""
    ai_report = data.get("ai_report")
    agg = data.get("aggregates") or {}

    out: list[str] = []
    name = case.get("name", case.get("case_id", "Case"))
    # A custom (admin-set) title_prefix wins; otherwise localise the default.
    title_prefix = tpl.get("title_prefix")
    if not title_prefix or title_prefix == TEMPLATE_DEFAULTS["title_prefix"]:
        title_prefix = L("title_prefix")
    out.append(f"# {title_prefix} — {name}")
    out.append("")
    if (tpl.get("header_md") or "").strip():
        out.append(tpl["header_md"].strip())
        out.append("")
    out.append(f"_{L('generated')} {_now()}_")
    if case.get("company"):
        out.append(f"_{L('company')}: {case['company']}_")
    out.append("")

    manifest = data.get("manifest") or {}
    if sections.get("manifest", True) and manifest:
        out.append(f"## {L('manifest_title')}")
        out.append("")
        out.append(f"_{L('manifest_blurb')}_")
        out.append("")
        if manifest.get("total_events"):
            mev = f"{int(manifest['total_events']):,}"
            out.append(f"- {L('m_events', n=mev)}")
        out.append(f"- {L('m_flagged', n=manifest.get('flagged_count', 0))}")
        if manifest.get("pinned_count"):
            out.append(f"- {L('m_pinned', n=manifest['pinned_count'])}")
        out.append(f"- {L('m_modules', hit=manifest.get('module_hit_run_count', 0), total=manifest.get('module_run_count', 0))}")
        if manifest.get("saved_search_count"):
            out.append(f"- {L('m_saved', n=manifest['saved_search_count'])}")
        if manifest.get("killchain_count"):
            out.append(f"- {L('m_killchains', n=manifest['killchain_count'])}")
        out.append(f"- {L('m_ai_yes', model=manifest.get('ai_model') or '?') if manifest.get('has_ai') else L('m_ai_no')}")
        out.append("")

    if sections.get("exec_summary", True):
        out.append(f"## {L('exec_summary')}")
        out.append("")
        if agg.get("total_events"):
            total_fmt = f"{int(agg['total_events']):,}"
            out.append(f"- **{L('total_events', n=total_fmt)}**")
        out.append(f"- **{L('flagged_review', n=len(flagged))}**")
        if agg.get("cti"):
            out.append(f"- **{L('cti_matched', n=len(agg['cti']))}**")
        if wl.get("hits"):
            out.append(f"- **{L('wl_hits', n=len(wl['hits']))}**")
        if pinned:
            out.append(f"- **{L('pinned_curated', n=len(pinned))}**")
        out.append("")

    if sections.get("overview", True) and agg:
        out.append(f"## {L('activity_overview')}")
        out.append("")
        out += _agg_rows_md(L("events_over_time"), agg.get("timeline") or [], L("u_events"))
        out += _agg_rows_md(L("artifact_types"), agg.get("artifact_types") or [])
        out += _agg_rows_md(L("top_src_ips"), agg.get("top_src_ips") or [], L("u_events"))

    modules = data.get("modules") or []
    if sections.get("modules", True) and modules:
        out.append(f"## {L('module_analysis')}")
        out.append("")
        for r in modules[:40]:
            lv = r.get("hits_by_level") or {}
            sev = ", ".join(f"{int(n)} {k}" for k, n in lv.items() if n)
            out.append(
                f"- **{r.get('module_id', '?')}** ({r.get('status', '')}) — "
                f"{r.get('total_hits', 0)} {L('u_hits')}" + (f" [{sev}]" if sev else "")
            )
        out.append("")

    saved = data.get("saved_searches") or []
    if sections.get("saved_searches", True) and saved:
        out.append(f"## {L('saved_title')}")
        out.append("")
        out.append(f"_{L('saved_blurb')}_")
        out.append("")
        for s in saved:
            out.append(f"- **{s.get('name', '?')}** — {L('saved_matches', n=int(s.get('count', 0)))}")
            if s.get("query"):
                out.append(f"  - `{s['query']}`")
            for ev in (s.get("samples") or [])[:3]:
                out.append(f"    - {_event_line(ev)}")
        out.append("")

    killchains = data.get("killchains") or []
    if sections.get("killchains", True) and killchains:
        out.append(f"## {L('killchains_title')}")
        out.append("")
        out.append(f"_{L('killchains_blurb')}_")
        out.append("")
        for kc in killchains:
            a = kc.get("anchor") or {}
            anchor_desc = a.get("summary") or a.get("fo_id", "?")
            out.append(f"### {L('kc_anchor')}: {anchor_desc}")
            if kc.get("tactics_covered"):
                out.append(f"_{L('kc_tactics')}: {', '.join(kc['tactics_covered'])}_")
            out.append("")
            for st in kc.get("steps", [])[:30]:
                ts = _ts(st.get("ts", ""))
                tac = st.get("tactic") or st.get("phase") or ""
                tech = st.get("technique") or ""
                summ = (st.get("summary") or "")[:200]
                out.append(f"- `{ts}` **{tac}** {tech} — {summ}")
            out.append("")

    if sections.get("threat_intel", True) and agg.get("cti"):
        out.append(f"## {L('threat_intel_matches')}")
        out.append("")
        for it in agg["cti"][:60]:
            ctx = []
            if it.get("count"):
                ctx.append(f"{it['count']} {L('u_hits')}")
            if it.get("last_seen"):
                ctx.append(f"{L('last')} {_ts(it['last_seen'])}")
            if it.get("feed"):
                ctx.append(f"{L('feed')} {it['feed']}")
            if it.get("threat"):
                ctx.append(str(it["threat"]))
            if it.get("prior_cases"):
                ctx.append(L("prior_cases", n=it["prior_cases"]))
            out.append(f"- `{it.get('value')}` ({it.get('type', 'ioc')}) — " + ", ".join(ctx))
        out.append("")

    if sections.get("ai_report", True) and ai_report and (ai_report.get("content") or "").strip():
        out.append("---")
        out.append("")
        out.append(f"## {L('ai_investigation')}")
        out.append("")
        meta_bits = []
        if ai_report.get("generated_at"):
            meta_bits.append(f"_{L('ai_generated')} {_ts(ai_report['generated_at'])}_")
        if ai_report.get("model_used"):
            meta_bits.append(f"_{ai_report['model_used']}_")
        if meta_bits:
            out.append(" · ".join(meta_bits))
            out.append("")
        ai_content = (ai_report["content"] or "").strip()
        ai_content = "\n".join(
            ("##" + ln[1:]) if ln.startswith("# ") else ln for ln in ai_content.splitlines()
        )
        out.append(ai_content)
        out.append("")

    if sections.get("pinned", True) and pinned:
        out.append(f"## {L('key_evidence')}")
        out.append("")
        for ev in pinned:
            out.append(f"- {_event_line(ev)}")
            if ev.get("pin_note"):
                out.append(f"  > _{ev['pin_note']}_")
        out.append("")

    max_flagged = int(tpl.get("max_flagged") or 50)
    if sections.get("flagged", True) and flagged:
        out.append(f"## {L('flagged_events')}")
        out.append("")
        for ev in flagged[:max_flagged]:
            out.append(f"- {_event_line(ev)}")
        if len(flagged) > max_flagged:
            out.append(f"_{L('and_more', n=len(flagged) - max_flagged)}_")
        out.append("")

    techs = mitre.get("techniques", []) if sections.get("mitre", True) else []
    if techs:
        out.append(f"## {L('mitre_coverage')}")
        out.append("")
        by_tactic: dict[str, list] = {}
        for t in techs:
            by_tactic.setdefault(t.get("tactic") or L("unknown"), []).append(t)
        for tactic, items in sorted(by_tactic.items()):
            out.append(f"### {tactic}")
            for t in sorted(items, key=lambda x: -x.get("count", 0)):
                out.append(f"- `{t['id']}` {t.get('name', '')} — {t.get('count', 0)} {L('u_event')}")
            out.append("")

    if sections.get("detections", True) and det.get("matches"):
        out.append(f"## {L('detections_fired')}")
        out.append("")
        out.append(f"_{L('last_run')}: {_ts(det.get('ran_at', ''))}_")
        for m in det["matches"]:
            rule = m.get("rule") or {}
            out.append(f"- **{rule.get('name', '?')}** ({m.get('match_count', 0)} {L('u_matches')})")
            if rule.get("description"):
                out.append(f"  > {rule['description']}")
        out.append("")

    if sections.get("watchlist", True) and wl.get("hits"):
        out.append(f"## {L('watchlist_hits')}")
        out.append("")
        out.append(f"_{L('auto_sweep')}: {_ts(wl.get('ran_at', ''))}_")
        for h in wl["hits"]:
            out.append(f"- **{h.get('label')}** (`{h.get('kind')}`) — {h.get('hits')} {L('u_matches')}")
            out.append(f"  - {L('query')}: `{h.get('query')}`")
        out.append("")

    if sections.get("notes", True) and notes:
        out.append(f"## {L('analyst_notes')}")
        out.append("")
        out.append(notes)
        out.append("")

    out.append("---")
    if (tpl.get("footer_md") or "").strip():
        out.append(tpl["footer_md"].strip())
    return "\n".join(out)


# ════════════════════════════ HTML (graphical) ════════════════════════════════


def _e(s) -> str:
    return html.escape(str(s if s is not None else ""))


def _inline_html(s: str) -> str:
    """Inline markdown marks on already-escaped text."""
    s = html.escape(s)
    s = re.sub(r"`([^`]+)`", r"<code>\1</code>", s)
    s = re.sub(r"\*\*([^*]+)\*\*", r"<strong>\1</strong>", s)
    s = re.sub(r"(?<!\w)_([^_]+)_(?!\w)", r"<em>\1</em>", s)
    s = re.sub(r"\[([^\]]+)\]\((https?://[^)]+)\)", r'<a href="\2">\1</a>', s)
    return s


def _md_to_html(md: str) -> str:
    """Small Markdown → HTML for free-text blocks (AI report, notes). Handles
    headings, lists, blockquotes, hr, inline marks AND pipe tables (the bit the
    old renderer dropped — markdown tables came out as literal `| a | b |`)."""
    lines = md.splitlines()
    out: list[str] = []
    in_list = False
    i = 0

    def _close_list():
        nonlocal in_list
        if in_list:
            out.append("</ul>")
            in_list = False

    while i < len(lines):
        line = lines[i].rstrip()
        # Pipe table: a header row followed by a |---|---| separator.
        if (
            line.lstrip().startswith("|")
            and i + 1 < len(lines)
            and re.match(r"^\s*\|?[\s:|-]+\|?\s*$", lines[i + 1])
            and "-" in lines[i + 1]
        ):
            _close_list()
            def _cells(row):
                row = row.strip().strip("|")
                return [c.strip() for c in row.split("|")]
            headers = _cells(line)
            out.append('<table><thead><tr>')
            out.extend(f"<th>{_inline_html(h)}</th>" for h in headers)
            out.append("</tr></thead><tbody>")
            i += 2
            while i < len(lines) and lines[i].lstrip().startswith("|"):
                cells = _cells(lines[i])
                out.append("<tr>" + "".join(f"<td>{_inline_html(c)}</td>" for c in cells) + "</tr>")
                i += 1
            out.append("</tbody></table>")
            continue
        if not line:
            _close_list()
            out.append("")
            i += 1
            continue
        for lvl, prefix in ((1, "# "), (2, "## "), (3, "### "), (4, "#### ")):
            if line.startswith(prefix):
                _close_list()
                out.append(f"<h{lvl}>{_inline_html(line[len(prefix):])}</h{lvl}>")
                break
        else:
            if line == "---":
                _close_list()
                out.append("<hr/>")
            elif line.startswith("  > ") or line.startswith("> "):
                _close_list()
                txt = line.split(">", 1)[1].strip()
                out.append(f"<blockquote>{_inline_html(txt)}</blockquote>")
            elif line.lstrip().startswith("- "):
                if not in_list:
                    out.append("<ul>")
                    in_list = True
                out.append(f"<li>{_inline_html(line.lstrip()[2:])}</li>")
            else:
                _close_list()
                out.append(f"<p>{_inline_html(line)}</p>")
        i += 1
    _close_list()
    return "\n".join(out)


def _stat_cards(cards: list[tuple[str, object]]) -> str:
    cells = "".join(
        f'<div class="card"><div class="num">{_e(v)}</div><div class="lbl">{_e(l)}</div></div>'
        for l, v in cards
    )
    return f'<div class="cards">{cells}</div>'


def _bar_chart(items: list[dict], unit: str = "") -> str:
    """Horizontal CSS bars for an aggregate (value/count list)."""
    items = [it for it in (items or []) if it.get("value") not in (None, "")][:12]
    if not items:
        return ""
    mx = max(int(it.get("count", 0)) for it in items) or 1
    rows = []
    for it in items:
        c = int(it.get("count", 0))
        pct = max(2, round(c / mx * 100))
        rows.append(
            f'<div class="bar-row"><span class="bar-lbl" title="{_e(it["value"])}">{_e(it["value"])}</span>'
            f'<span class="bar-track"><span class="bar-fill" style="width:{pct}%"></span></span>'
            f'<span class="bar-val">{c:,}{(" " + unit) if unit else ""}</span></div>'
        )
    return f'<div class="bars">{"".join(rows)}</div>'


def _module_table(runs: list[dict], L: Labels) -> str:
    """Analysis-module run results — what scanners ran and what they found."""
    rows = []
    for r in runs[:40]:
        lv = r.get("hits_by_level") or {}
        sev = " ".join(
            f'<span class="tag">{int(n)} {k}</span>'
            for k, n in (("high", lv.get("high")), ("medium", lv.get("medium")), ("low", lv.get("low")))
            if n
        )
        rows.append(
            "<tr>"
            f"<td>{_e(r.get('module_id', '?'))}</td>"
            f"<td>{_e(r.get('status', ''))}</td>"
            f"<td>{_e(r.get('total_hits', 0))}</td>"
            f"<td>{sev or '<span class=muted>—</span>'}</td>"
            f"<td class=mono>{_e(_ts(r.get('started_at', '')))}</td>"
            "</tr>"
        )
    return (
        f"<table class=mod><thead><tr><th>{_e(L('h_module'))}</th><th>{_e(L('h_status'))}</th>"
        f"<th>{_e(L('h_hits'))}</th><th>{_e(L('h_by_severity'))}</th><th>{_e(L('h_run_at'))}</th>"
        "</tr></thead><tbody>"
        + "".join(rows)
        + "</tbody></table>"
    )


def _cti_table(items: list[dict], L: Labels) -> str:
    """Enriched threat-intel match table: indicator + context (when/how often/
    feed/threat-type/prior cases) instead of a wall of equal '1 hit' bars."""
    rows = []
    for it in items[:60]:
        prior = it.get("prior_cases")
        prior_cell = (
            f'<span class="tag">{_e(L("prior_cases", n=int(prior)))}</span>'
            if prior else f'<span class="muted">{_e(L("first_seen_lbl"))}</span>'
        )
        rows.append(
            "<tr>"
            f"<td class=mono>{_e(it.get('value'))}</td>"
            f"<td>{_e(it.get('type', ''))}</td>"
            f"<td>{_e(it.get('count', 0))}</td>"
            f"<td class=mono>{_e(_ts(it.get('first_seen', '')))}</td>"
            f"<td class=mono>{_e(_ts(it.get('last_seen', '')))}</td>"
            f"<td>{_e(it.get('feed', '') or '—')}</td>"
            f"<td>{_e(it.get('threat', '') or '—')}</td>"
            f"<td>{prior_cell}</td>"
            "</tr>"
        )
    return (
        f"<table class=cti><thead><tr><th>{_e(L('h_indicator'))}</th><th>{_e(L('h_type'))}</th>"
        f"<th>{_e(L('h_hits'))}</th><th>{_e(L('h_first_seen'))}</th><th>{_e(L('h_last_seen'))}</th>"
        f"<th>{_e(L('h_feed'))}</th><th>{_e(L('h_threat'))}</th><th>{_e(L('h_history'))}</th>"
        "</tr></thead><tbody>" + "".join(rows) + "</tbody></table>"
    )


def _event_table(events: list[dict], cap: int, L: Labels) -> str:
    rows = []
    for ev in events[:cap]:
        f = _ev_fields(ev)
        rows.append(
            "<tr>"
            f"<td class=mono>{_e(f['ts'])}</td>"
            f"<td><span class=tag>{_e(f['type'])}</span></td>"
            f"<td class=mono>{_e(f['host'])}</td>"
            f"<td class=mono>{_e(f['user'])}</td>"
            f"<td class=mono>{_e(f['src'])}</td>"
            f"<td>{_e(f['msg'])}{('<div class=note>' + _e(f['note']) + '</div>') if f['note'] else ''}</td>"
            "</tr>"
        )
    extra = f'<div class="muted">{_e(L("and_more", n=len(events) - cap))}</div>' if len(events) > cap else ""
    return (
        f"<table class=evt><thead><tr><th>{_e(L('h_time'))}</th><th>{_e(L('h_type'))}</th>"
        f"<th>{_e(L('h_host'))}</th><th>{_e(L('h_user'))}</th><th>{_e(L('h_source'))}</th><th>{_e(L('h_message'))}</th>"
        "</tr></thead><tbody>"
        + "".join(rows)
        + "</tbody></table>"
        + extra
    )


_HTML_CSS = """
:root{--ink:#0F172A;--mut:#64748B;--line:#E5E7EB;--accent:#6366F1;--bg:#fff;}
*{box-sizing:border-box}
body{font:14px/1.55 -apple-system,BlinkMacSystemFont,'Segoe UI',Inter,sans-serif;color:var(--ink);max-width:980px;margin:2em auto;padding:0 1.2em;background:var(--bg);}
h1{font-size:1.9em;letter-spacing:-.02em;border-bottom:1px solid var(--line);padding-bottom:.4em;}
h2{font-size:1.3em;margin-top:2em;letter-spacing:-.01em;}
h3{font-size:1.05em;color:#4B5563;margin:1.2em 0 .4em;}
h4{font-size:.98em;color:#4B5563;}
.meta{color:var(--mut);font-size:.9em;}
code{background:#F3F4F6;padding:.1em .35em;border-radius:3px;font-family:'JetBrains Mono',Consolas,monospace;font-size:.9em;}
.mono{font-family:'JetBrains Mono',Consolas,monospace;font-size:.85em;color:#334155;white-space:nowrap;}
blockquote{border-left:3px solid var(--accent);margin:.5em 0;padding:.2em 1em;color:#4B5563;background:#F9FAFB;}
hr{border:none;border-top:1px solid var(--line);margin:2em 0;}
.cards{display:flex;flex-wrap:wrap;gap:.7em;margin:1em 0;}
.card{flex:1 1 120px;border:1px solid var(--line);border-radius:10px;padding:.8em 1em;background:#FAFAFB;}
.card .num{font-size:1.5em;font-weight:700;color:var(--accent);}
.card .lbl{font-size:.78em;color:var(--mut);margin-top:.2em;text-transform:uppercase;letter-spacing:.03em;}
.bars{margin:.4em 0 1.2em;}
.bar-row{display:flex;align-items:center;gap:.6em;margin:.18em 0;font-size:.85em;}
.bar-lbl{width:200px;flex-shrink:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-family:'JetBrains Mono',monospace;color:#334155;}
.bar-track{flex:1;background:#F1F5F9;border-radius:4px;height:14px;overflow:hidden;}
.bar-fill{display:block;height:100%;background:linear-gradient(90deg,#818CF8,#6366F1);}
.bar-val{width:96px;flex-shrink:0;text-align:right;color:var(--mut);font-variant-numeric:tabular-nums;}
table{border-collapse:collapse;width:100%;margin:.6em 0 1.4em;font-size:.86em;}
th,td{border:1px solid var(--line);padding:.4em .6em;text-align:left;vertical-align:top;}
th{background:#F8FAFC;font-weight:600;color:#334155;}
tr:nth-child(even) td{background:#FCFCFD;}
table.evt td:last-child{white-space:normal;}
.tag{background:#EEF2FF;color:#4338CA;border-radius:4px;padding:.05em .45em;font-size:.85em;}
.note{color:var(--mut);font-style:italic;font-size:.92em;margin-top:.2em;}
.muted{color:var(--mut);font-size:.85em;margin:.3em 0;}
@media print{body{max-width:100%;margin:0;}.card{background:#fff;}}
"""


def render_html(data: dict, tpl: dict | None = None, language: str | None = None) -> str:
    tpl = tpl or dict(TEMPLATE_DEFAULTS)
    L = Labels(language)
    sections = tpl.get("sections") or {}
    case = data.get("case") or {}
    pinned = data.get("pinned") or []
    flagged = data.get("flagged") or []
    mitre = data.get("mitre") or {}
    wl = data.get("watchlist") or {}
    det = data.get("detections") or {}
    notes = data.get("notes") or ""
    ai_report = data.get("ai_report")
    agg = data.get("aggregates") or {}
    name = case.get("name", case.get("case_id", "Case"))
    max_flagged = int(tpl.get("max_flagged") or 50)

    title_prefix = tpl.get("title_prefix")
    if not title_prefix or title_prefix == TEMPLATE_DEFAULTS["title_prefix"]:
        title_prefix = L("title_prefix")

    b: list[str] = []
    b.append(f"<h1>{_e(title_prefix)} — {_e(name)}</h1>")
    if (tpl.get("header_md") or "").strip():
        b.append(_md_to_html(tpl["header_md"].strip()))
    meta = [f"{_e(L('generated'))} {_e(_now())}"]
    if case.get("company"):
        meta.append(f"{_e(L('company'))}: {_e(case['company'])}")
    b.append(f'<p class="meta">{" · ".join(meta)}</p>')

    manifest = data.get("manifest") or {}
    if sections.get("manifest", True) and manifest:
        b.append(f"<h2>{_e(L('manifest_title'))}</h2>")
        b.append(f'<p class="meta">{_e(L("manifest_blurb"))}</p>')
        items = []
        if manifest.get("total_events"):
            items.append(_e(L("m_events", n=f"{int(manifest['total_events']):,}")))
        items.append(_e(L("m_flagged", n=manifest.get("flagged_count", 0))))
        if manifest.get("pinned_count"):
            items.append(_e(L("m_pinned", n=manifest["pinned_count"])))
        items.append(_e(L("m_modules", hit=manifest.get("module_hit_run_count", 0), total=manifest.get("module_run_count", 0))))
        if manifest.get("saved_search_count"):
            items.append(_e(L("m_saved", n=manifest["saved_search_count"])))
        if manifest.get("killchain_count"):
            items.append(_e(L("m_killchains", n=manifest["killchain_count"])))
        items.append(_e(L("m_ai_yes", model=manifest.get("ai_model") or "?")) if manifest.get("has_ai") else _e(L("m_ai_no")))
        b.append("<ul>" + "".join(f"<li>{x}</li>" for x in items) + "</ul>")

    cti = agg.get("cti") or []
    if sections.get("exec_summary", True):
        b.append(f"<h2>{_e(L('exec_summary'))}</h2>")
        # Only the cards that actually carry signal — no 0-valued noise for
        # things the investigation didn't use (pinned/MITRE) and detections the
        # analyst doesn't want surfaced here.
        cards = []
        if agg.get("total_events"):
            cards.append((L("c_events"), f"{int(agg['total_events']):,}"))
        cards.append((L("flagged_events"), len(flagged)))
        if cti:
            cards.append((L("threat_intel_matches"), len(cti)))
        if wl.get("hits"):
            cards.append((L("watchlist_hits"), len(wl["hits"])))
        if pinned:
            cards.append((L("key_evidence"), len(pinned)))
        b.append(_stat_cards(cards))

    if sections.get("overview", True) and agg:
        b.append(f"<h2>{_e(L('activity_overview'))}</h2>")
        if agg.get("timeline"):
            b.append(f"<h3>{_e(L('events_over_time'))}</h3>")
            b.append(_bar_chart(agg["timeline"], L("u_events")))
        if agg.get("artifact_types"):
            b.append(f"<h3>{_e(L('artifact_types'))}</h3>")
            b.append(_bar_chart(agg["artifact_types"]))
        if agg.get("top_src_ips"):
            b.append(f"<h3>{_e(L('top_src_ips'))}</h3>")
            b.append(_bar_chart(agg["top_src_ips"], L("u_events")))
        if agg.get("severity"):
            b.append(f"<h3>{_e(L('severity'))}</h3>")
            b.append(_bar_chart(agg["severity"]))

    modules = data.get("modules") or []
    if sections.get("modules", True) and modules:
        b.append(f"<h2>{_e(L('module_analysis'))}</h2>")
        b.append(f'<p class="meta">{_e(L("module_blurb"))}</p>')
        b.append(_module_table(modules, L))

    saved = data.get("saved_searches") or []
    if sections.get("saved_searches", True) and saved:
        b.append(f"<h2>{_e(L('saved_title'))}</h2>")
        b.append(f'<p class="meta">{_e(L("saved_blurb"))}</p>')
        rows = "".join(
            f"<tr><td>{_e(s.get('name', '?'))}</td>"
            f"<td>{_e(L('saved_matches', n=int(s.get('count', 0))))}</td>"
            f"<td class=mono>{_e(s.get('query', ''))}</td></tr>"
            for s in saved
        )
        b.append(
            f"<table><thead><tr><th>{_e(L('h_label'))}</th><th>{_e(L('h_hits'))}</th>"
            f"<th>{_e(L('h_query'))}</th></tr></thead><tbody>{rows}</tbody></table>"
        )

    killchains = data.get("killchains") or []
    if sections.get("killchains", True) and killchains:
        b.append(f"<h2>{_e(L('killchains_title'))}</h2>")
        b.append(f'<p class="meta">{_e(L("killchains_blurb"))}</p>')
        for kc in killchains:
            a = kc.get("anchor") or {}
            anchor_desc = a.get("summary") or a.get("fo_id", "?")
            b.append(f"<h3>{_e(L('kc_anchor'))}: {_e(anchor_desc)}</h3>")
            if kc.get("tactics_covered"):
                b.append(f'<p class="meta">{_e(L("kc_tactics"))}: {_e(", ".join(kc["tactics_covered"]))}</p>')
            rows = "".join(
                "<tr>"
                f"<td class=mono>{_e(_ts(st.get('ts', '')))}</td>"
                f"<td>{_e(st.get('phase', '') or st.get('tactic', ''))}</td>"
                f"<td>{_e(st.get('tactic', ''))}</td>"
                f"<td>{_e(st.get('technique', ''))}</td>"
                f"<td>{_e((st.get('summary') or '')[:200])}</td>"
                "</tr>"
                for st in kc.get("steps", [])[:30]
            )
            b.append(
                f"<table class=evt><thead><tr><th>{_e(L('h_time'))}</th><th>{_e(L('h_phase'))}</th>"
                f"<th>{_e(L('h_tactic'))}</th><th>{_e(L('h_technique'))}</th><th>{_e(L('h_summary'))}</th>"
                f"</tr></thead><tbody>{rows}</tbody></table>"
            )

    if sections.get("threat_intel", True) and cti:
        b.append(f"<h2>{_e(L('threat_intel_matches'))}</h2>")
        b.append(f'<p class="meta">{_e(L("threat_intel_blurb"))}</p>')
        b.append(_cti_table(cti, L))

    if sections.get("ai_report", True) and ai_report and (ai_report.get("content") or "").strip():
        b.append(f"<hr/><h2>{_e(L('ai_investigation'))}</h2>")
        mb = []
        if ai_report.get("generated_at"):
            mb.append(f"{_e(L('ai_generated'))} {_e(_ts(ai_report['generated_at']))}")
        if ai_report.get("model_used"):
            mb.append(_e(ai_report["model_used"]))
        if mb:
            b.append(f'<p class="meta">{" · ".join(mb)}</p>')
        ai_content = (ai_report["content"] or "").strip()
        ai_content = "\n".join(
            ("##" + ln[1:]) if ln.startswith("# ") else ln for ln in ai_content.splitlines()
        )
        b.append(_md_to_html(ai_content))

    if sections.get("pinned", True) and pinned:
        b.append(f"<h2>{_e(L('key_evidence'))}</h2>")
        b.append(_event_table(pinned, 500, L))

    if sections.get("flagged", True) and flagged:
        b.append(f"<h2>{_e(L('flagged_events'))}</h2>")
        b.append(_event_table(flagged, max_flagged, L))

    techs = mitre.get("techniques", []) if sections.get("mitre", True) else []
    if techs:
        b.append(f"<h2>{_e(L('mitre_coverage'))}</h2>")
        by_tactic: dict[str, list] = {}
        for t in techs:
            by_tactic.setdefault(t.get("tactic") or L("unknown"), []).append(t)
        for tactic, items in sorted(by_tactic.items()):
            b.append(f"<h3>{_e(tactic)}</h3>")
            b.append(
                _bar_chart(
                    [{"value": f"{t['id']} {t.get('name', '')}".strip(), "count": t.get("count", 0)}
                     for t in sorted(items, key=lambda x: -x.get("count", 0))]
                )
            )

    if sections.get("detections", True) and det.get("matches"):
        b.append(f"<h2>{_e(L('detections_fired'))}</h2>")
        b.append(f'<p class="meta">{_e(L("last_run"))}: {_e(_ts(det.get("ran_at", "")))}</p>')
        rows = "".join(
            f"<tr><td>{_e((m.get('rule') or {}).get('name', '?'))}</td>"
            f"<td>{_e(m.get('match_count', 0))}</td>"
            f"<td>{_e((m.get('rule') or {}).get('description', ''))}</td></tr>"
            for m in det["matches"]
        )
        b.append(
            f"<table><thead><tr><th>{_e(L('h_rule'))}</th><th>{_e(L('h_matches'))}</th>"
            f"<th>{_e(L('h_description'))}</th></tr></thead><tbody>{rows}</tbody></table>"
        )

    if sections.get("watchlist", True) and wl.get("hits"):
        b.append(f"<h2>{_e(L('watchlist_hits'))}</h2>")
        b.append(f'<p class="meta">{_e(L("auto_sweep"))}: {_e(_ts(wl.get("ran_at", "")))}</p>')
        rows = "".join(
            f"<tr><td>{_e(h.get('label'))}</td><td><span class=tag>{_e(h.get('kind'))}</span></td>"
            f"<td>{_e(h.get('hits'))}</td><td class=mono>{_e(h.get('query'))}</td></tr>"
            for h in wl["hits"]
        )
        b.append(
            f"<table><thead><tr><th>{_e(L('h_label'))}</th><th>{_e(L('h_kind'))}</th>"
            f"<th>{_e(L('h_hits'))}</th><th>{_e(L('h_query'))}</th></tr></thead><tbody>{rows}</tbody></table>"
        )

    if sections.get("notes", True) and notes:
        b.append(f"<h2>{_e(L('analyst_notes'))}</h2>")
        b.append(_md_to_html(notes))

    b.append("<hr/>")
    if (tpl.get("footer_md") or "").strip():
        b.append(f'<p class="meta">{_md_to_html(tpl["footer_md"].strip())}</p>')

    page = (
        "<!doctype html><html><head><meta charset=utf-8/>"
        f"<title>{_e(name)} — Citadel report</title><style>{_HTML_CSS}</style>"
        f"</head><body>{''.join(b)}</body></html>"
    )
    # Charset-proof: emit pure ASCII with non-ASCII (—, ·, accents…) as numeric
    # HTML entities, so the document renders correctly no matter how it's served
    # or embedded (download, iframe, srcdoc, proxy). Fixes the "â€"" mojibake.
    return page.encode("ascii", "xmlcharrefreplace").decode("ascii")
